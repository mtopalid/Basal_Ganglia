...
file = folder + '/Weights_dStr.npy'
load = np.load(file)
MeanWeights = (load.mean(axis = 0) - Wmin) / (Wmax - Wmin)
t = np.arange(MeanWeights[:,0].shape[0])
StdWeights = load.std(axis = 0)
file = folder+ '/MeanWeights-dStr.npy'
np.save(file, MeanWeights)
...
plt.ylabel("Weights")
plt.xlabel("Trial number")
temp_title = title + 'Cognitive Cortex to Cognitive Striatum (D1)'
plt.title(temp_title)
#plt.ylim(0.4,0.80)
...
file = folder + '/Weights_iStr.npy'
load = np.load(file)
MeanWeights = (load.mean(axis = 0) - Wmin) / (Wmax - Wmin)
t = np.arange(MeanWeights[:,0].shape[0])
StdWeights = load.std(axis = 0)
file = folder+ '/MeanWeights-iStr.npy'
np.save(file, MeanWeights)
...
plt.ylabel("Weights")
plt.xlabel("Trial number")
temp_title = title + 'Cognitive Cortex to Cognitive Striatum (D2)'
plt.title(temp_title)
#plt.ylim(0.4,0.80)
file = folder + "/Weights-iStr.png"
fig.savefig(file)
