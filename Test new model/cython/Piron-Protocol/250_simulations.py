...
if __name__ == "__main__":
	...
	for i in range(simulations):
		...
		f.write('\n\n--------Testing Familiar without GPi--------')
		Ptest_fam_NoGPi[i,:], RTtest_fam_NoGPi[i,:], Dtest_fam_NoGPi[i], APtest_fam_NoGPi[i,:], mBctest_fam_NoGPi[i], ABCtest_fam_NoGPi[i], NoMovetest_fam_NoGPi[i] = testing_trials(trials = n_testing_trials, f = f, debugging = False, save = True)
		print

		print '--------Testing UnFamiliar without GPi--------'
		f.write('\n--------Testing UnFamiliar without GPi--------')
		Ptest_unfam_NoGPi[i,:], RTtest_unfam_NoGPi[i,:], Dtest_unfam_NoGPi[i], APtest_unfam_NoGPi[i,:], mBctest_unfam_NoGPi[i], ABCtest_unfam_NoGPi[i], NoMovetest_unfam_NoGPi[i] = testing_trials(trials = n_testing_trials, f = f, debugging = False, save = True, familiar = False)
		...
		Ptest_unfam[i,:], RTtest_unfam[i,:], Dtest_unfam[i], RPtest_unfam[i,:], APtest_unfam[i,:], mBctest_unfam[i], ABCtest_unfam[i], NoMovetest_unfam[i] = learning_trials(trials = n_testing_trials, f = f, debugging = False, save = True, familiar = False, type = 'testing', W_COG = W_COG, W_MOT = W_MOT, W_STR = W_STR)
		...
		f.write('\n\n--------Testing Familiar with GPi--------')
		Ptest_fam[i,:], RTtest_fam[i,:], Dtest_fam[i], RPtest_fam[i,:], APtest_fam[i,:], mBctest_fam[i], ABCtest_fam[i], NoMovetest_fam[i] = learning_trials(trials = n_testing_trials, f = f, debugging = False, save = True, type = 'testing', W_COG = W_COG, W_MOT = W_MOT, W_STR = W_STR)
		print
