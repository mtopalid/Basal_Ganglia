...
def display_all(history, duration=3.0, filename=None):
    ...
    ax = subplot(7,3,7)
    ax.set_ylabel("dStriatum", fontsize=24)
    for i in range(4):
        plt.plot(timesteps, history["dSTR"]["mot"][:,i], c='k', lw=.5)
    ax.set_xticks([])

    ax = subplot(7,3,8)
    for i in range(4):
        plt.plot(timesteps, history["dSTR"]["cog"][:,i], c='k', lw=.5)
    ax.set_xticks([])

    ax = subplot(7,3,9)
    for i in range(16):
        plt.plot(timesteps, history["dSTR"]["ass"][:,i], c='k', lw=.5)
    ax.set_xticks([])

    ax = subplot(7,3,10)
    ax.set_ylabel("iStriatum", fontsize=24)
    for i in range(4):
        plt.plot(timesteps, history["iSTR"]["mot"][:,i], c='k', lw=.5)
    ax.set_xticks([])

    ax = subplot(7,3,11)
    for i in range(4):
        plt.plot(timesteps, history["iSTR"]["cog"][:,i], c='k', lw=.5)
    ax.set_xticks([])

    ax = subplot(7,3,12)
    for i in range(16):
        plt.plot(timesteps, history["iSTR"]["ass"][:,i], c='k', lw=.5)
    ax.set_xticks([])
