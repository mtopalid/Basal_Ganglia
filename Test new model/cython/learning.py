...
def learning_trials(inversable = False, inverse_all = True, inverse_trial = 50, f = None, hist = False, trials = n_trials, debugging = True, save = False, protocol = 'Piron', familiar = True, type = 'learning', W_COG = None, W_MOT = None, W_dSTR = None, W_iSTR = None):
	trained = False
	while not trained:
		...
		wdStr = np.zeros((trials,n))
		wiStr = np.zeros((trials,n))
		reset(protocol = protocol, W_COG = W_COG, W_MOT = W_MOT, W_dSTR = W_dSTR, W_iSTR = W_iSTR)
		inverse = False
		for j in range(20):
			...
			wdStr[j,:] = connections["CTX.cog -> dSTR.cog"].weights
			wiStr[j,:] = connections["CTX.cog -> iSTR.cog"].weights
		...
		for i in range(j+1, trials):
			...
			if inversable:
				if i == inverse_trial:
					if inverse_all:
						CUE["reward"] = rewards_Guthrie_inverse_all
					else:
						CUE["reward"] = rewards_Guthrie_inverse_middle
					inverse = True
			if debugging:
				print 'Trial: ', i + 1
			if hist:
				histor, RT, P, D, RP, AP, mBc, ABC, NoMove = learning(inverse = inverse, inverse_all = inverse_all, f = f, trial_n = i, debugging = debugging, protocol = protocol, familiar = familiar, hist = hist, P = P, D = D, mBc = mBc, ABC = ABC, NoMove = NoMove, RT = RT, RP = RP, AP = AP)
			else:
				RT, P, D, RP, AP, mBc, ABC, NoMove = learning(inverse = inverse, inverse_all = inverse_all, f = f, trial_n = i, debugging = debugging, protocol = protocol, familiar = familiar, hist = hist, P = P, D = D, mBc = mBc, ABC = ABC, NoMove = NoMove, RT = RT, RP = RP, AP = AP)
			wCog[i,:] = connections["CTX.cog -> CTX.ass"].weights
			wMot[i,:] = connections["CTX.mot -> CTX.ass"].weights
			wdStr[i,:] = connections["CTX.cog -> dSTR.cog"].weights
			wiStr[i,:] = connections["CTX.cog -> iSTR.cog"].weights
	debug(f = f, RT = RT, P = P, D = D, RP = RP, AP = AP, mBc = mBc, ABC = ABC, NoMove = NoMove)
	debug_learning(wCog[i,:], wMot[i,:], wdStr[i,:], wiStr[i,:], cues_value = CUE["value"], f = f)
	if save:
		return P, RT, np.array(D).mean(), RP, AP, np.array(mBc).mean(), np.array(ABC).mean(), len(NoMove)/float(n_trials), wCog, wMot, wdStr, wiStr
	if hist:
		return histor, P
	else:
		return P
