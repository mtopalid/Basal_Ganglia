...
connections = {
    ...
    "CTX.mot -> CTX.ass" : MotToAss( CTX.mot.V, CTX.ass.Isyn, np.ones(4) ),
}
...
def process(n=2, learning=True, P = [], D = [], AP = np.zeros(n), R = [], RP = np.zeros(n)):
    ...
    if learning:
		...
		#dw = np.minimum(alpha_LTP**2 * CTX.cog.V[choice],0.001)
		#connections["CTX.cog -> CTX.ass"].weights[choice] += dw
		#dw = np.minimum(alpha_LTP**2 * CTX.mot.V[mot_choice],0.001)
		#print dw
		#connections["CTX.mot -> CTX.ass"].weights[mot_choice] += dw
